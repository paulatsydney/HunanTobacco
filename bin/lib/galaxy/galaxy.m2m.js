...
GLOBAL.labels.set({
  galaxyAPI: {
    showProgress: { 
      ...
      de:"Show progress" },
    setBusy: { 
      en:"Enable busy state", 
      fr:"Activer l'état occupé", 
      de:"Enable busy state" },
    showConsole: { 
      en:"Show debug console", 
      fr:"Afficher la console de test", 
      de:"Show debug console" },
    sendDiagnostic: { 
      en:"Send diagnostic", 
      fr:"Envoyer un diagnostic", 
      de:"Send diagnostic" },
    selfTestMode: { 
      en:"Enable self test mode", 
      fr:"Activer le mode self-test", 
      de:"Enable self test mode" },
    reinitialize: { 
      en:"Reinitialize connection", 
      fr:"Réinitialiser la connexion", 
      de:"Reinitialize connection" }
  }
});
...
//	* @path ctx.options.galaxyAPI.debug */ debug: false,
	...
	* @path ctx.options.galaxyAPI.enableStudioRestart */ enableStudioRestart: false,
	...
	/** Saves attended jobs on server
	* @property {boolean} saveAttendedJobs
	* @path ctx.options.galaxyAPI.saveAttendedJobs */ saveAttendedJobs: true,
...
//	};
	...
	*/
	 ...
	 * @path ctx.m2m.receiveSocketStatus
	 * @param {ctx.event} ev 
	 * @return {boolean} result
	 */
	...
	*/
	...
	_pProcess.addOn({ evSocketStatus: function(ev) {
		self.receiveSocketStatus(ev);
	}});
	_pProcess.events['evSocketStatus'].internal = true;
...
ctx.galaxyAPI = (function () {
	...
	var _agentUidKey = "HKCU\\Software\\SAP\\Intelligent RPA\\Settings\\AgentUid";
	var _projectFolder = "%localappdata%\\SAP\\Intelligent RPA\\Projects";
...
//	var _saveAvailableProjects = function (availableProjects) {
//		var filename = _projectFolder + "\\AvailableProjects.json";
//		availableProjects = availableProjects || [];
//		// remove existing
//		if (!ctx.fso.folder.exist(_projectFolder)) {
//			ctx.fso.folder.create(_projectFolder);
//		}
//		if (ctx.fso.file.exist(filename)) {
//			ctx.fso.file.remove(filename);
//		}
//		var txt = ctx.json.stringify(availableProjects);
//		ctx.fso.file.write(filename, txt, e.file.encoding.UTF8);
//		return true;
//	}
	
//	var _readAvailableProjects = function () {
//		var filename = _projectFolder + "\\AvailableProjects.json";
//		var availableProjects = [];
//		try {
//			var txt = ctx.fso.file.read(filename, e.file.encoding.UTF8);
//			var obj = ctx.json.parse(txt);
//			if (obj && obj.data && obj.data.availableProjects) {
//				availableProjects = obj.data.availableProjects;
//			} else if (Array.isArray(obj)) {
//				availableProjects = obj;
//			}
//		} catch (ex) {		}
//		// TODO : read currentProject
	...
	*/
	...
	_executeClientCommand[_clientCommand.notifyJob] = function(obj, source, resObj, answer) {
		...
		if (job.id && _pendingNotifications[job.id] && ('function' === typeof _pendingNotifications[job.id])) {
			_pendingNotifications[job.id](job.code, job.label, job);
	...
	_executeClientCommand[_clientCommand.updateClientInfos] = function(updateClientInfos, source, resObj, answer) {
		...
		if (updateClientInfos && ('object' === typeof updateClientInfos)) {
			if (updateClientInfos.availableProjects) {
				...
				//_saveAvailableProjects(updateClientInfos.availableProjects);
				//ctx.saveProjectList(updateClientInfos.availableProjects);

				// If not already on the project
				//ctx.log("updateClientInfos received");

				// GDLV restart on updateClientInfos
				var packageVersionUid = ctx.registry.get('Software\\SAP\\Intelligent RPA\\Desktop Agent\\CurrentProject', e.registry.root.CurrentUser);
				//if ((!_packageVersionUid) || (_availableProjects[0] && _availableProjects[0].packageVersionUid == _packageVersionUid)) {
				if ((!_packageVersionUid) || (packageVersionUid && packageVersionUid == _packageVersionUid)) {
					// update server with the new list (no need to restart)
					ctx.log("No change, staying on: " + _packageVersionUid);
					ctx.galaxyAPI.sendProjectInfos ();
				} else {
					ctx.log("Changing from " + _packageVersionUid + " to " + _availableProjects[0].packageVersionUid);
					ctx.wait(function(ev) {
						// Restart Contextor if the first project is not the running project, or has changed
						if (!ctx.switchOrShutdown(false, true)) {
							// update server with the new list (no need to restart)
							ctx.galaxyAPI.sendProjectInfos ();
						}
					}, 500);
	...
	_executeClientCommand[_clientCommand.runJob] = function(obj, source, resObj, answer) {
		...
		ctx.wait(function(ev) {
			//_showProgressPopup(jobData, true);
...
/**
		...
		*/
		sendProjectInfos : function () {
			...
			if (_availableProjects && _availableProjects[0]) {
				_packageVersionUid = projectContent.packageVersionUid = _availableProjects[0].packageVersionUid;
				_configurationUid = projectContent.configurationUid = _availableProjects[0].configurationUid;
			}
		...
		*/
		setBusy : function (state, callback) {
			...
			if (ctx.m2m.isConnected()) {
				...
				res = self.sendCommand((state ? _serverCommand.setBusy : _serverCommand.setIdle), null, busyCallback);
			} else {
				if ('function' === typeof callback) {
					callback(e.error.OK, '');
				}
...
GLOBAL.events.START.on(function (ev) {
	...
	ctx.galaxyAPI.init();
	ctx.m2m.init();
	ctx.options.galaxyAPI.enableStudioRestart = true;
	//var pckLocal = '8b71a06c-f70e-4553-8025-69a8a0643a16';
	//var pckSpace = 'c3d6b44e-50ee-40e5-98a6-353505ed88b7';
//	var txt = ctx.fso.file.open(ctx.options.path.local + '../../availableProjets', e.file.encoding.UTF8);
//	var availableProjets = ctx.json.parse(txt);
//	var availableProjet = availableProjets[0];
	//var obj = JSON.parse('{ "name":"John", "age":30, "city":"New York"}');
});

GLOBAL.events.INIT.on(function (ev) {
	ctx.wait(function(ev) {
		if (ctx.options.m2m.enabled /*&& (ctx.options.m2m.url || ctx.options.m2m.useWebSockets)*/) {
			if (ctx.options.m2m.showMenu && systray) {
				// create a test menu
				systray.addMenu('', 'M2MTestMenu', 'Galaxy API');
				
				systray.addMenu('M2MTestMenu', 'evM2MProgressPopup', GLOBAL.labels.galaxyAPI.showProgress, '', function(ev) {
					ctx.galaxyAPI.toggleOption('showProgress');
					systray.check('evM2MProgressPopup', ctx.options.galaxyAPI.showProgress);
				});
				systray.check('evM2MProgressPopup', ctx.options.galaxyAPI.showProgress);

				systray.addMenu('M2MTestMenu', 'evM2MSendDiag', GLOBAL.labels.galaxyAPI.sendDiagnostic, '', function(ev) {
					ctx.galaxyAPI.addJob({
						scenario: GLOBAL.scenarios.GetDiagnostic,
						target: ctx.galaxyAPI.getM2MId(),
						data: null,
				...
				systray.addMenu('M2MTestMenu', 'evM2MSetBusy', GLOBAL.labels.galaxyAPI.setBusy, function(ev) {
					ctx.galaxyAPI.setBusy(!ctx.galaxyAPI.isBusy(), function(code) {
						systray.check('evM2MSetBusy', ctx.galaxyAPI.isBusy());
					});
				});	
				
				systray.addMenu('M2MTestMenu', 'evM2MReinitialize', GLOBAL.labels.galaxyAPI.reinitialize, '', function(ev) {
					//ctx.galaxyAPI.reinitialize();
					systray.check('evM2MSetBusy', ctx.galaxyAPI.isBusy());
				});
				
				if (ctx.options.isDebug || ctx.options.m2m.debug) {
					systray.addMenu('M2MTestMenu', 'evM2MShow', GLOBAL.labels.galaxyAPI.showConsole, '', function(ev) {
						ctx.m2m.toggleOption('showConsole');
						ctx.m2m.reopenM2MPage();
						systray.check('evM2MShow', ctx.options.m2m.showConsole);
					});
					systray.check('evM2MShow', ctx.options.m2m.showConsole);

					systray.addMenu('M2MTestMenu', 'evM2MSelfTestMode', GLOBAL.labels.galaxyAPI.selfTestMode, '', function(ev) {
						ctx.galaxyAPI.toggleOption('selfTestMode');
						systray.check('evM2MSelfTestMode', ctx.options.galaxyAPI.selfTestMode);
					});
					systray.check('evM2MSelfTestMode', ctx.options.galaxyAPI.selfTestMode);
				} 
