...
//  else
//    this.data = ctx.dataManager(); // no model provided
//  else if (('function' === typeof this.dataModel) && this.dataModel.prototype && this.dataModel.prototype.constructor)
//    this.data = new this.dataModel();
//  else if ('function' === typeof this.dataModel)
//    this.data = this.dataModel();
//  else if ('object' === typeof this.dataModel)
//    ctx.set(this.dataModel, this.data);
...
//  /** scenario synchronous state
//  * @path ctx.scenarioClass.synchronous
//  * @ignore
//  * @property {boolean} */ this.synchronous = false;
  ...
  this.clear = function (init, cancel) {
    ...
    if (init) {
      ...
      if (cancel) {
	        ...
	        this.job.label = this.label = 'Job was rejected';
      ...
      if (typeof ctx.galaxyAPI !== 'undefined' && this.main) {
        // save job on server
        //ctx.galaxyAPI.addJob(this.job);
				if (this.job.attended && ctx.options.galaxyAPI && ctx.options.galaxyAPI.saveAttendedJobs) {
    ...
    */
    this.start = function (data, sourceEvent, job) {
      ...
      if (job && (job instanceof ctx.jobClass)) sc.job = job;
			...
			if ((typeof ctx.galaxyAPI !== 'undefined') && (sc.main) && sc.job.attended && ctx.options.galaxyAPI && ctx.options.galaxyAPI.saveAttendedJobs ) {
