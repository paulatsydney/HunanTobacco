...
ctx.ajax = (function _CtxAjax() {
  ...
  */
  ...
  function _createMultipartData( formData, boundary ) {
    ...
    streamBin.SaveToFile(ctx.options.path.log + '\\test.txt', 2); // for debug purpose
    streamBin.Close();
    return res;
  }
  ...
  error: function(xhr, status, statusText) {
    ...
    */
    call : function (params) {
      ...
      if (params.header && ('object' === typeof params.header) && !params.headers) 
...
//          //if (options.password) _xhr.setRequestHeader('x-pass', options.password);
        ...
        // update body length
//        if (body && body.length) {
//          options.xhr.setRequestHeader("Content-length", body.length);
//        }
